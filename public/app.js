class SoldierSignOutApp {
    ...
    hidePinModal() {
        ...
        // Clear the purpose and reset modal title and content
        delete this.pinModal.dataset.purpose;
        const modalTitle = this.pinModal.querySelector('.modal-header h2');
        if (modalTitle) {
            modalTitle.textContent = 'Sign In Confirmation';
        }
        
        // Reset the message text for sign-ins
        const modalMessage = this.pinModal.querySelector('#pinModalMessage');
        if (modalMessage) {
            modalMessage.textContent = 'Please enter your PIN to sign in this group:';
        }
        
        // Show the sign-in details again for regular sign-ins
    ...
    requestPinForSettings() {
        ...
        // Hide the sign-in details for settings access
        if (this.signInDetails) {
        ...
        // Update modal title and message for settings access
        const modalTitle = this.pinModal.querySelector('.modal-header h2');
        if (modalTitle) {
            modalTitle.textContent = 'Settings Access';
            console.log('Modal title updated');
        } else {
            console.error('Modal title element not found!');
        }
        
        // Update the message text for settings
        const modalMessage = this.pinModal.querySelector('#pinModalMessage');
        if (modalMessage) {
            modalMessage.textContent = 'Please enter your PIN to access system settings:';
        }
